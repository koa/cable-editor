...
- `components/label_printer.rs`: `PrintLabelButton` connects, waits for the reported tape and opens a dialog asking for the cable diameter (last value in `localStorage`) with a live preview; label rendering on a canvas. Text height is capped at 70% of the diameter, the label is cropped to the glyphs.
